import {Action} from '@ngrx/store';
import { STORE_DEVTOOLS_CONFIG } from '@ngrx/store-devtools/src/config';
import { Recipes } from '../recipes.model';

export const SET_RECIPES='[Recipes] SET_RECIPES';
export const FETCH_RECIPES='[Recipes] FETCH_RECIPES';
export const ADD_RECIPE='[Recipes] ADD_RECIPE';
export const UPDATE_RECIPE='[Recipes] UPDATE_RECIPE';
export const DELETE_RECIPE='[Recipes] DELETE_RECIPE';
export const STORE_RECIPE='[Recipes] STORE_RECIPE';
//export const STORE_SINGLE_RECIPE='[Recipes] STORE_SINGLE_RECIPE';

export class SetRecipes implements Action{
    readonly type=SET_RECIPES;

    constructor(public payload:Recipes[]){}
}
export class FetchRecipes implements Action{
    readonly type = FETCH_RECIPES;
}

export class AddRecipe implements Action{
    readonly type=ADD_RECIPE;

    constructor(public payload:Recipes){}
}

export class UpdateRecipe implements Action {
    readonly type = UPDATE_RECIPE;

    constructor(public payload:{index:number;newRecipe:Recipes}){}
}

export class DeleteRecipe implements Action{
    readonly type=DELETE_RECIPE;

    constructor(public payload:number){}
}

export class StoreRecipe implements Action{
    readonly type=STORE_RECIPE;
}
// export class StoreSingleRecipe implements Action{
//     readonly type=STORE_SINGLE_RECIPE;

//     constructor(public payload:{index:number,updatedRecipe:Recipes}){}
// }

export type RecipeActions = 
SetRecipes
|FetchRecipes
|AddRecipe
|UpdateRecipe
|DeleteRecipe
|StoreRecipe;
